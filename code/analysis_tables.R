...
cohort %>%
  ...
  select("Cohort bracket"=cohort_cat, 
         ...
         "Rural pop. lowest %"= l_rural, 
         "Rural pop. highest %"=h_rural) %>% 
...
modelsummary(list(
  ...
  coef_map = c("educ_w" = "Mean education (yrs)$^a$",
               "educ_gini" = "Education Gini$^b$",
               "n_ethnicity" = "No. of ethnic groups",
               "n_sample" = "Sample size$^c$",
               "l_gen.eth_n" = "Group size lowest",
               "h_gen.eth_n" = "Group size highest",  
               "africaAfrica" = "Africa",
               "cohort_cat1970-1979" = "Cohort 1970-1979$^d$",
               "cohort_cat1980-" = "Cohort 1980-$^d$", 
               "(Intercept)" = "(Intercept)"),
  stars = stars, 
  gof_map = gm,
  shape = "rcollapse",
