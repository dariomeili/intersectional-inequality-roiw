...
honduras <- bind_rows(dfs, .id = "data_name") %>% 
...
honduras %<>% 
  mutate(ethnicity = fct_collapse(v131eth,
                                  "Other indigenous" = c("tolup?n", "Tolup?n","pech (paya)", "Pech (paya)", "misquito", "Misquito", 
                                                         "Nahoa", "nahoa", "Tawaka (sumo)", "tawaka (sumo)", "Garifuna", "garifuna"),
                                  "Lenca" =c("Lenca", "lenca"),
                                  "Maya chorti" = c("Maya chorti", "maya chorti"),
                                  "None" = c("dk/none", "DK/none"), 
                                  "Tawaka (sumo)" = c(),
                                  "Other" = c("negro ingl?s", "Negro ingl?s", "other", "Other")
  )
  ) 
...
honduras %<>%
  mutate(religion = fct_collapse(v130rel, 
                                 ...
                                 "Evangelical/protestant" = c("evangelical / protestant", "Evangelical / Protestant"), 
                                 "Other/No religion" = c("No religion", "no religion", "other", "Other")
  ))
...
honduras %<>% 
  select(data_name,
         ...
         "education" = v133educ)
# write data set
df <-  bind_rows(df, honduras)
rm(meta, dfs, honduras)
