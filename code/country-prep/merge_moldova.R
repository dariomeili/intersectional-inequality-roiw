...
# harmonize year
dfs
dfs[["MBIR53FL"]]$v007year <- 2005
dfs[["MBMR52FL"]]$v007year <- 2005

moldova <- bind_rows(dfs, .id = "data_name") %>% 
...
moldova %<>%
  mutate(religion = fct_collapse(v130rel, 
                                 "other christian" = c("catholic/greek cath.", 
                                                       "protestant (ex. evangelical, baptist, jehovah witness)",
                                                       "catholic/greek cath",
                                                       "protestant"),
                                 "other/no religion" = c("jewish", "no religion", "other", "dk", "refused/dk")
  ))
