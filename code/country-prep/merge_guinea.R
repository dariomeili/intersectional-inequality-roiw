...
# harmonize year
dfs
dfs[["GNIR41FL"]]$v007year <- 1999
dfs[["GNMR41FL"]]$v007year <- 1999

guinea <- bind_rows(dfs, .id = "data_name") %>% 
...
guinea %<>% 
  mutate(ethnicity = fct_collapse(v131eth,
                                  "soussou" = c("sousou", "soussou"),
                                  "malinké" = c("malinké", "malinke"),
                                  "guerzé" = c("guerze", "guerzé"),
                                  "other" = c("other", "stranger", "toma")
         )
  ) 
...
guinea %<>%
  mutate(religion = fct_collapse(v130rel, 
                                 "other/no religion" = c("animist", "no religion",  "other")
  ))
