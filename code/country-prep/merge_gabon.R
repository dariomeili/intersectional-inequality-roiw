...
gabon <- bind_rows(dfs, .id = "data_name") %>% 
...
gabon %<>% 
  mutate(ethnicity = fct_collapse(v131eth,
                                  other = c("other", "pygmee", "okande-tsogho", "myene"))
  ) 
...
gabon %<>%
  mutate(religion = fct_collapse(v130rel, 
                                 "other/no religion" = c("animist", "other", "no religion")
  ))
