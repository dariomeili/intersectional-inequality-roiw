...
guyana <- bind_rows(dfs, .id = "data_name") %>% 
  mutate(country = "guyana", 
         country_code = "GY")
...
guyana %<>% 
  mutate(ethnicity = fct_collapse(v131eth,
                                  "mixed/other" = c("portuguese", "chinese", "mixed", "other")
  )
  ) 
...
guyana %<>%
  mutate(religion = fct_collapse(v130rel, 
                                 "other" = c("muslim", "rastafarian", "not religious", "other")
  ))
