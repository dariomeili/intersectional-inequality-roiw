...
sao_tome <- bind_rows(dfs, .id = "data_name") %>% 
  mutate(country = "sao_tome", 
         country_code = "ST")
...
sao_tome %<>%
  mutate(religion = fct_collapse(v130rel, 
                                 "Catholic" = c("catholic"), 
                                 "Evangelic/other christian" = c("evangelist", "aventist", "jeova witness",
                                                                 "new apostolic", "mana"),
                                 "Other/no religion" = c("muslum", "none", "not responded", "other"), 
  ))
...
sao_tome %<>% 
  select(data_name,
         ...
         "education" = v133educ)
# write data set
df <-  bind_rows(df, sao_tome)
rm(meta, dfs, sao_tome)
