...
namibia <- bind_rows(dfs, .id = "data_name") %>% 
...
namibia %<>% 
  mutate(ethnicity = fct_collapse(v131eth,
                                  "other" = c("english", "tswana", "other", "san", "caprivi languages")
...
namibia %<>%
  mutate(religion = fct_collapse(v130rel, 
                                 ...
                                 "Protestant" = c("protestant/anglican", "protestant", "elcin", "seventh-day adventist"),
